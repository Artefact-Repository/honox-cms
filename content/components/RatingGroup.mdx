...
Designed strictly to visualise aggregated ratings. Hover interactions and click selections are disabled, rendering a clean, read-only display.
...
- **WAI-ARIA Accessibility:** Complies with the WAI-ARIA rating widget pattern. It utilises an accessible hidden input to support standard HTML form submissions, while managing `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` states on the container.
