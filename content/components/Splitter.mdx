...
This component is available as a `splitter` block in the [Page Builder](/docs/PageBuilder) (`content/pages/*.json`). **Always renders static** (`interactive={false}`, no drag-to-resize) — a panel's `content` is arbitrary nested JSX, which can't cross the island's prop-serialization boundary the way `children` can:
