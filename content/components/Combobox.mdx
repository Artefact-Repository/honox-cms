...
- **Smart Adaptive Hydration (Tier 2):** To optimise initial page speeds, the Combobox compiles to static, zero-JS HTML/CSS markups on server loads, downloading interactive JS payloads only when custom signals are provided or when `"interactive": true` is explicitly passed.
