...
- **Zero-Layout Shift Performance:** Styles compile down to optimised, static Panda CSS utilities, avoiding client-side Javascript measurement overhead.
...
- **Responsive JSON Strings:** When authoring pages within Sveltia CMS, responsive values (like breakpoint column configurations) should be written as JSON-serialised strings (e.g. `'{"base": 1, "md": 2}'`). These are automatically parsed and translated to breakpoint styles at runtime.
