...
| Search box (both pages) | **No new block needed** — the existing `search` block type (`page-registry.tsx:1058`, already registered in `config.yml`) already covers `src` (`/api/projects/search.json` / `/api/tasks/search.json` — endpoints already exist), `placeholder`, `action`, `filterAttribute`, `emptyStateId` field-for-field (confirmed against the blog task's identical search box, `blog-page-contentisation.md`); only the per-locale `placeholder` text needs to move into content |
