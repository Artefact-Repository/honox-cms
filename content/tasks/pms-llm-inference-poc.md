...
3. Ran the harness end-to-end in headless Chromium (Playwright, `--enable-unsafe-webgpu --use-angle=swiftshader`, software-rendered — no real GPU available in that environment) and got three data points, in order:
   ...
   - `Qwen2.5-1.5B-Instruct-q4f16_1-MLC`: fit under the quota (829MB) and loaded fully, but generation failed immediately: `Extension f16 is not allowed on the Device` — the `q4f16_1` quantisation needs WebGPU's `shader-f16` feature, which SwiftShader's software rasterizer doesn't expose. Virtually all real GPUs do.
