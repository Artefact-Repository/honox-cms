...
Add a prominent sticky top banner ("Draft preview — not published" / localised) when `post.draft` is true on the detail page (and surface a drafts-only view on the listing for non-prod, e.g. a "Show drafts" toggle). Guard so drafts never appear in production listings, search index, tag/author archives, or the sitemap/feed regardless of the banner. Keep the localised strings (zh/de/fr/es/pt) consistent with the existing `currentLocale === "zh" ? ... : ...` pattern. Low effort, prevents accidental "is this live?" confusion during editorial review.
