...
1. **`app/utils/project-save.ts`** — add `deleteProject(slug)` mirroring `deleteTask` in `app/utils/task-save.ts`: import `fetchFile` + `deleteFile` from `./git-backend` (currently only `createFile`, `fileExists`, `requireToken` are imported) and do `requireToken()` → `fetchFile('content/projects/${slug}.md', token)` → `deleteFile(path, file.sha, 'Delete project "${slug}"', token)`.
